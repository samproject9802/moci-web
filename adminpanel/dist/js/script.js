...
function addtocart(nama,harga,qty,minsale,diskon,valid,invalid) {
	...
	if (currentdate >= validdiskon && currentdate <= invaliddiskon) {
		...
		if (!qty) {
			...
			var total = (hargaproduk * quantity) - 35000;
			var selisihdis = ((total/(hargaproduk * quantity)) * 100);
			discount = 100 - selisihdis;
			var discnow = discount.toFixed(2);
			$('#salestable').append("<tr class='prd'><td class='item'>"+namaproduk+"</td><td class='price'>"+hargaproduk+"</td><td class='qty'>"+quantity+"</td><td class='disc'>"+discnow+" %</td><td class='subtotal'>"+total+"</td><td class='action'><button class='btn btn-danger btn-sm' type='button' id='delete-row'><i class='fas fa-times-circle'></i></button><tr>");
			GrandTotal();
		} else if (qty >= 1) {
			var realpriceproduk = hargaproduk - 6000;
			var totalsebdisc = (hargaproduk * quantity);
			var totalandisc = (realpriceproduk * quantity);
			discount = ((totalsebdisc - totalandisc) / totalsebdisc) * 100;
			var discnow = discount.toFixed(2);
			$('#salestable').append("<tr class='prd'><td class='item'>"+namaproduk+"</td><td class='price'>"+hargaproduk+"</td><td class='qty'>"+quantity+"</td><td class='disc'>"+discnow+" %</td><td class='subtotal'>"+totalandisc+"</td><td class='action'><button class='btn btn-danger btn-sm' type='button' id='delete-row'><i class='fas fa-times-circle'></i></button><tr>");
			GrandTotal();
		} else {
			discount = 0;
			salediskon = 0;
